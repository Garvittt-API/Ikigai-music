<div align="center">

<img src="logo.png" alt="Ikigai Logo" width="160" />

<br/>

# IKIGAI

### *Your music. Your way.*

<br/>

<a href="https://github.com/Garvittt-API/Ikigai-music/releases/latest/download/IkigaiPlayer-Setup.exe">
<img src="https://img.shields.io/badge/⬇️_DOWNLOAD_FOR_WINDOWS-000?style=for-the-badge&logo=windows&logoColor=white&labelColor=000" />
</a>

<br/>

<img src="https://img.shields.io/badge/v0.11.0-blue?style=flat-square" />
<img src="https://img.shields.io/badge/Windows-00ADEF?style=flat-square&logo=windows&logoColor=white" />
<img src="https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black" />
<img src="https://img.shields.io/badge/macOS-000000?style=flat-square&logo=apple&logoColor=white" />

</div>

<br/>

---

<br/>

<div align="center">

## 🎧 What is Ikigai?

Ikigai *(生き甲斐)* — the Japanese art of finding joy in life.

**Ikigai Player** is a music player that respects your library,
looks beautiful, and just works.

No subscriptions. No ads. No cloud dependency.
**Your music, on your terms.**

</div>

<br/>

---

<br/>

<div align="center">

## ✨ Features

</div>

<br/>

<table>
<tr>
<td width="50%" align="center">

### 🎵 Stream From Anywhere
**Local files** · **Jellyfin** · **Navidrome** · **YouTube Music** · **SoundCloud**

One app. Every source. Switch freely.

<br/>

### 🎨 Beautiful Themes
Fully customizable dark themes with
dynamic color extraction from album art.

Your library sets the mood.

<br/>

### 🎛️ Studio-Grade Sound
10-band equalizer with presets.
Channel mode. Crossfade.
Sound the way it was meant to be heard.

</td>
<td width="50%" align="center">

### 📝 Real-Time Lyrics
Synced lyrics that scroll with the music.
Plain text mode too.
Never miss a word.

<br/>

### ⚡ Lightning Fast
Built with **Rust**. Opens instantly.
Handles 10,000+ tracks without breaking a sweat.

<br/>

### 🎮 Stay Connected
**Discord Rich Presence** — show what you're playing.
**System Tray** — minimize and keep listening.
**Global Hotkeys** — control from anywhere.

</td>
</tr>
</table>

<br/>

---

<br/>

<div align="center">

## 🔥 And There's More

</div>

<br/>

<div align="center">

| | | |
|:---:|:---:|:---:|
| 📥 **yt-dlp Downloads** | 🎭 **Mini Player** | 🎪 **Internet Radio** |
| 🔀 **Crossfade** | 📊 **Listening Stats** | 🎤 **Scrobble to ListenBrainz** |
| 🌍 **30+ Languages** | 🎪 **Genre Browsing** | 🔍 **Quick Search** |
| 📁 **Smart Playlists** | ⭐ **Favorites Sync** | 🎭 **Album Art Colors** |

</div>

<br/>

---

<br/>

<div align="center">

## 🖥️ Built Different

<br/>

<table>
<tr>
<td align="center" width="120">

**⚡**

*Built in Rust*
*Runs native*
*No Electron*

</td>
<td align="center" width="120">

**🎨**

*Clean UI*
*Custom themes*
*Smooth animations*

</td>
<td align="center" width="120">

**🔌**

*5 backends*
*1 app*
*Zero compromise*

</td>
<td align="center" width="120">

**🔒**

*No tracking*
*No ads*
*Open source*

</td>
</tr>
</table>

</div>

<br/>

---

<br/>

<div align="center">

## 🚀 Quick Setup — Connect YouTube Music

</div>

<br/>

<div align="center">

<table>
<tr>
<td width="50%" align="left">

### Step 1 — Install
Download `IkigaiPlayer-Setup.exe` and run it.
Follow the wizard — takes less than 30 seconds.

<br/>

### Step 2 — Open Settings
Launch Ikigai Player from your Start Menu.
Click **Settings** in the sidebar.

<br/>

### Step 3 — Add Media Server
Scroll down until you see **Media Servers**.
Click **Add Servers**.

</td>
<td width="50%" align="left">

### Step 4 — Choose YouTube Music
A dropdown appears with **Jellyfin** selected by default.
Click on it and select **YouTube Music**.

<br/>

### Step 5 — Name Your Server
Enter server name. you can name it anything you want (custom)
Click **Next**.

<br/>

### Step 6 — Sign In
Your browser opens automatically.
Sign in with your Google account. and then restart "Ikigai"
That's it — **you're ready to go!** 🎉

</td>
</tr>
</table>

</div>

<br/>

---

<br/>

<div align="center">

## 🔧 Troubleshooting

</div>

<br/>

<div align="center">

**After logging in through brower still you see nothing after searching songs in search menu?**

1. Go to **Settings**
2. Scroll down to **Media Servers**
3. then if you see "connected" then no need to change anything
4. but if you see "disconnecrted" and click the small **Login** button next to it
4. then you will see **Connected** appear
5. If it doesn't show connected after clicking Login, [open an issue on GitHub](https://github.com/Garvittt-API/Ikigai-player/issues) or email us at **garvitchoudhary2315@gmail.com**

</div>

<br/>

---

<br/>

<div align="center">

## ⬇️ Get Started

<br/>

**[📥 Download for Windows](https://github.com/Garvittt-API/Ikigai-music/releases/latest/download/IkigaiPlayer-Setup.exe)**

<br/>

*Just run the installer. Done in 30 seconds.*

<br/>

---

<br/>

<img src="logo.png" alt="Ikigai" width="40" />

*Made with ❤️ by Garvittt*

</div>
